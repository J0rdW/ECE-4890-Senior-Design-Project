...
void lidarStart(){
    ...
    if (lidar.getCurrentPoint().startBit) {
      ...
      // Angle check
      lidarSection = angleSelection(angleAtMinDist);
      
      printData(angleAtMinDist, minDistance, lidarSection);  // Print the previous data
      minDistance = 100000;  // Reset the data
      angleAtMinDist = 0;    // Reset the data
    } else {
      // Otherwise if the distance is within LIDAR's range, set the distance and angle
